...
本文适合已经能打开 DSH、但从未在里面使用过 Codex 的读者。如果你还没有运行过 DSH，可以先从文末的官方项目说明开始。
...
以下步骤在 DSH 0.1.1-rc.2 和 Codex 插件 0.1.2 上验证通过。DSH 仍在快速开发，不同版本可能不兼容，所以本文固定了版本。

Node.js 是 DSH 运行时需要的基础环境，pnpm 是 DSH 安装插件时使用的下载工具。你不需要先学会它们，只要确认电脑中已经有可用版本。

开始前，请确认：

1. 在终端里输入 `node --version`，显示 22.13 或更高版本；
2. 在终端里输入 `pnpm --version`，能够看到版本号；
...
```bash
npx @deepseek-ai/dsh@0.1.1-rc.2 plugin --profile web add relay-dsh-plugin-codex@0.1.2
```

这条命令的作用很简单：为 DSH 的网页界面安装 Codex 插件。第一次运行时需要下载文件，稍等一会儿即可。

命令运行结束后，继续下一步。真正的验收标准不是终端里出现了多少文字，而是 DSH 菜单中能否看到 Codex。
...
如果重启后仍然没有，可以在终端输入下面的命令检查插件是否已经安装：

```bash
npx @deepseek-ai/dsh@0.1.1-rc.2 plugin --profile web why relay-dsh-plugin-codex
```
...
这个插件适合已经在使用 DSH，又希望在同一个页面里完成 Codex 编程任务的人。它不会让 Codex 突然变得更聪明，也不会自动安排多个 AI 互相协作。
