...
Codex、Claude Code 和 Cursor 已经把模型、代码检索、命令执行、文件修改、审批和产品界面组合好了。普通用户不需要先理解插件宿主和兼容关系，就能开始任务。
...
我们最初也修改过 DSH Fork，把终端、文件浏览和会话后端直接放进核心代码。短期很快，但官方一更新，合并冲突就会持续出现，用户也无法在官方版本上按需选择功能。

后来这些能力被拆成 Codex、Claude Code、Workbench、Files、Terminal 等独立插件。插件之间通过 DSH 的公开机制交互，而不是直接引用彼此内部代码。
...
如果你只想马上完成编码任务，直接选择 Codex、Claude Code 或 Cursor。

如果你同时使用几个 Agent，已经遇到同一项目会话散落的问题，可以研究 DSH 加对应会话插件，但先从一个项目、一个插件开始。

如果你正在开发 Agent 产品，关注自定义 Agent、插件机制和不同会话后端，DSH 的开放边界会比“哪次模型排名更高”更值得研究。

反过来，如果你只需要一个 Agent，也没有跨工具管理问题，就不必安装额外 Harness。
...
官方 DSH，适合先了解 Harness 本身：
https://github.com/deepseek-ai/deepseek-harness

Codex 插件，用于在 DSH 中运行和继续 Codex Thread：
https://github.com/yangbobo2021/relay-dsh-plugin-codex

Claude Code 插件，用于在 DSH 中运行和继续 Claude Code Session：
https://github.com/yangbobo2021/relay-dsh-plugin-claude

Relay，包含完整插件关系和后续事件协作研究：
