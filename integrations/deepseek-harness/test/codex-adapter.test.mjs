...
test("Codex interactions use DSH approval and question services, while waits use the main Relay runtime", async () => {
  const agent = fakeAgent();
  const adapter = { dshSessionForThread: threadId => threadId === "thread-1" ? agent.id : null };
  const calls = { approvals: [], questions: [], waits: [] };
  const ctx = {
  ...
  const relayRuntime = {
    async registerWaits(input) { calls.waits.push(input); return input; },
    cancelWaits() {},
  };
  const runtime = new InteractionRuntime();

  await handleCodexServerRequest(ctx, {
    adapter, relayRuntime, runtime,
  ...
  assert.equal(calls.waits[0].sessionId, agent.id);
  assert.equal(calls.waits[0].waits[0].expected_event, "build.ready");
...
test("the production event router delivers exact external event types and DSH accepts durable Codex activity", async () => {
  ...
  const router = createExactEventRouter();
  const session = {
    session_id: "dsh-1",
    waits: [{ wait_id: "wait-1", status: "active", exclusive: true, expected_event: "build.ready" }],
  };
  const matched = await router.route({ event: { type: "build.ready" }, sessions: [session] });
  const missed = await router.route({ event: { type: "other" }, sessions: [session] });
  assert.equal(matched.disposition, "deliver");
  assert.equal(matched.deliveries[0].session_id, "dsh-1");
  assert.equal(missed.disposition, "dismiss");
});
