...
//#endregion
//#region ../codex/app-server-client.mjs
...
//#endregion
//#region ../codex/session-runtime.mjs
		...
		inputSchema: {
			...
			properties: {
				event_type: {
					type: "string",
					minLength: 1,
					description: "Stable external event type to match."
				},
				description: {
					type: "string",
					minLength: 1,
					description: "What work should continue after the event."
				}
			}
		}
	},
...
async function handleCodexServerRequest(ctx, { adapter, relayRuntime, runtime, request }) {
...
var CodexDshAdapter = class extends LlmAdapter {
	constructor({ runtime, ready, linkStore = null, attachments = null, logger = console, dynamicTools = CODEX_DYNAMIC_TOOLS }) {
		super();
	...
	async ensureThread(sessionId) {
		...
		const operation = this.createOrResumeThread(key).finally(() => {
...
var RelayCodexTerminalGateway = class extends TypertRemoteService {
	...
	async list(request) {
		return success([...this.terminals.values()].filter((terminal) => terminal.sessionId === request.sessionId).sort((left, right) => left.createdAt - right.createdAt).map((terminal) => this.snapshot(terminal)));
	}
	async spawn(request) {
		try {
	...
	owned(request) {
		...
		if (!terminal || terminal.sessionId !== request.sessionId) return rejected("terminal-not-found", `terminal "${request.terminalId}" was not found`);
...
function createDshCodexPlugin(ctx, core, config = {}) {
	return definePlugin({
		manifest: {
			id: "relay.dsh.codex",
			version: "1.0.0",
		...
		async activate({ capabilities, defer }) {
			...
			const events = core.capabilities.optional("relay.events.v1");
			const workspace = core.capabilities.require("relay.dsh.workspace.v1");
