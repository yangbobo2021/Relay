...
export function createDshCodexPlugin(ctx, core, config = {}) {
  return definePlugin({
    manifest: {
    ...
    async activate({ capabilities, defer }) {
      ...
      defer(runtime.subscribeRequest((request) => {
        void handleCodexServerRequest(ctx, { adapter, relayRuntime: events, runtime, request })
      ...
      if (terminal) await activateCordisScope(ctx, defer, "relay Codex terminal remote", (scope) => {
        new RelayCodexTerminalGateway(scope, { terminal, resolveAgent: workspace.resolveAgent });
      });
