 ...
 * eight child slots (declaration = exclusive render authority), seats the
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    // there); these eight are the frame's children, declared by the same
    ...
    /** Ordered tab contributions for the Codex-style Auxiliary Workspace. */
    'workbench.side.tabs': { kind: 'list'; scope: 'root'; owner: WorkbenchSideTabsOwnerProps }
    /** Ordered add-view menu contributions for the Auxiliary Workspace. */
    'workbench.side.menu': { kind: 'list'; scope: 'root'; owner: WorkbenchSideMenuOwnerProps }
    /** Active Auxiliary Workspace content, dispatched by registered view id. */
    'workbench.side.view': { kind: 'keyed'; scope: 'root'; owner: WorkbenchPanelOwnerProps }
    /** Codex-style bottom workbench panel occupied by the local terminal. */
...
// through the owner-share intersection (PropsRuntime & PropsRenderSlots &
...
/** Shared owner contract for docked workbench surfaces. */
export interface WorkbenchPanelOwnerProps {
  closePanel(): void
}

/** Owner controls shared by every Auxiliary Workspace tab contribution. */
export interface WorkbenchSideTabsOwnerProps extends WorkbenchPanelOwnerProps {
  activeView: string
  activateView(viewId: string): void
}

/** Owner controls shared by every Auxiliary Workspace add-menu contribution. */
export interface WorkbenchSideMenuOwnerProps {
  activeView: string
  activateView(viewId: string): void
}
  ...
  ctx.effect(() => {
    ...
    const disposeRegistration = ctx.slots.register({
      ...
      children: {
        ...
        'workbench.side.tabs': { kind: 'list', scope: 'root' },
        'workbench.side.menu': { kind: 'list', scope: 'root' },
        'workbench.side.view': { kind: 'keyed', scope: 'root' },
        'workbench.bottom.terminal': { kind: 'single', scope: 'root' },
        'shell.overlay': { kind: 'list', scope: 'root' },
      },
