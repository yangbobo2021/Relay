...
// One artifact supports audited DSH prereleases through 0.1.2-rc.1; runtime probes verify this range.
...
test("plugins and shared libraries are independently publishable workspace packages", async () => {
  ...
  for (const directory of ["integrations/dsh-files", "integrations/dsh-terminal"]) {
    ...
    assert.equal(manifest.peerDependencies?.["relay-dsh-plugin-workbench"], "0.2.2",
      `${directory} must declare the Workbench peer contract`);
    assert.equal(manifest.devDependencies?.["relay-dsh-plugin-workbench"], "github:yangbobo2021/relay-dsh-plugin-workbench#43a2c501c7d51927dd8e18e7aa6bf32fd0db0bf5",
  ...
  for (const directory of ["integrations/codex", "integrations/claude", "integrations/events", "integrations/semantic-router", "integrations/monitors", "integrations/monitor-time", "integrations/monitor-process", "integrations/monitor-author", "integrations/github", "integrations/email", "integrations/dsh-plugin-manager"]) {
    const patch = await readFile(join(root, directory, "cordis.patch.yml"), "utf8");
