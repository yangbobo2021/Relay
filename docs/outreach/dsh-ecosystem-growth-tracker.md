...
| `docs/media/dsh-plugin-manager-codex-install-demo.en.mp4` | Real 40.03-second isolated run starting at the first meaningful English input, with native English DSH UI, English conversation, and English event captions: exact Codex package search, plan-only safety boundary, explicit confirmation, npm install, and final `succeeded` status. Full decode, frame-zero, black-frame, visible-CJK, and privacy QA passed. | Current Reddit and official-community version; linked from the Plugin Manager and Codex English READMEs |
