...
官方 `alpha.3` 源码构建成功。随后直接从 npm registry 下载十个已发布的 `0.2.1` 包，
...
与已发布版本同源的 Manager `0.2.1` 包还单独完成安装、禁用、启用、升级和卸载流程；
每次冷启动均加载预期版本，未确认操作不会执行，确认令牌不能重放。npm 上的 Manager
`0.2.1` tarball 也在 `manager-only` 和 `all-plugins` 场景中完成实际下载、安装、启动和浏览器加载。

十个插件还直接链接官方 `alpha.3` 声明与构建产物执行各自的 `npm run verify`，10/10 通过；
...
十个已发布插件 `0.2.1` 与 DSH `0.1.2-alpha.3` 在现有自动化覆盖的安装、加载、组合和核心
功能范围内兼容，不需要修改运行时代码。
